...
describe("VMs", () => {
    ...
    describe("Setup and run a ceremony using VMs", () => {
        ...
        it("should terminate the VM(s) when finalizing the ceremony", async () => {
            ...
            // call the function
            await expect(finalizeCircuit(userFunctions, ceremonyClosed.uid, circuit.uid, ceremonyBucket, `handle-id`))
                .to.be.fulfilled
            ...
            for (const instanceId of instancesToTerminate) await expect(checkIfRunning(ec2, instanceId)).to.be.rejected
        })
    })
})
